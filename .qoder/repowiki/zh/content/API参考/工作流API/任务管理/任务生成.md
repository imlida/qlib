...
## 更新摘要
**已更新内容**
- 更新了 `RollingGen.generate` 方法中 `test_end` 的处理逻辑，以支持 `None` 值（表示“直到现在”）
- 更新了 `handler_mod` 函数以正确处理 `test_seg_end_time` 为 `None` 的情况
- 添加了对 `transform_end_date` 工具函数的引用和说明
- 更新了相关章节以反映对开放式时间边界的支持
...
- **新增**：详细说明对开放式时间边界（test_end 为 None）的支持，增强系统对动态或持续更新数据集的健壮性
...
- **新增：对开放式时间边界的支持**
  - `RollingGen.generate` 方法现在使用 `transform_end_date` 函数处理 `test_end`。
  - 当 `test_end` 为 `None`、`-1` 或超出日历范围时，会自动将其设置为日历中的最后一个交易日。
  - 这使得系统能够正确处理“直到现在”的开放式时间边界，增强了对动态或持续更新数据集的健壮性。
...
- **更新**：现在能正确处理 `test_seg_end_time` 为 `None` 的情况（即“直到现在”），并相应地更新 `handler_end_time`。
- 安全性：对不存在键或类型异常进行捕获，避免报错

章节来源
- [gen.py](file://qlib/workflow/task/gen.py#L93-L126)
- [__init__.py](file://qlib/utils/__init__.py#L468-L489)
...
- 数据加载
  - 使用 handler_mod 自动延长处理器 end_time，避免因数据边界导致的任务失败
- 批量生成
  - task_generator 支持多生成器串联，建议按功能拆分生成器，提高可维护性
- **新增：开放式时间边界最佳实践**
  - 当处理持续更新的数据流时，将任务模板的 `test` 段结束时间设置为 `None`，以启用“直到现在”的行为。
  - 确保 `handler_mod` 被启用（默认行为），以便自动将数据处理器的 `end_time` 延长至最新的 `test` 段结束时间。
...
## 结论
任务生成系统围绕 TaskGen 抽象基类展开，通过 RollingGen 与 MultiHorizonGenBase 实现滚动与多时长任务生成。TimeAdjuster 提供了对齐、截断与平移的基础设施，配合 handler_mod 与 trunc_segments，有效防止未来信息泄露。借助 task_generator，用户可以灵活组合多种生成策略，构建大规模、可复用的任务流水线。在实践中，合理选择复制策略与截断长度，是保障实验稳定性的关键。**本次更新增强了系统对开放式时间边界的支持**，通过 `transform_end_date` 函数和 `handler_mod` 的改进，使工作流系统在处理动态或持续更新的数据集时更加健壮和灵活。